...
func matchAssetSelection(assets []releaseAsset, pattern, arch string) assetSelection {
...
func gitHubReleaseAssetCandidates(matches []assetMatch) []GitHubReleaseAssetCandidate {
	candidates := make([]GitHubReleaseAssetCandidate, 0, len(matches))
	for _, match := range matches {
		candidates = append(candidates, GitHubReleaseAssetCandidate{
			Name:        match.name,
...
func formatAssetCandidateNames(candidates []GitHubReleaseAssetCandidate) string {
