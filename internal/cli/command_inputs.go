...
func completeManagedAppIDs(cmd *cobra.Command, toComplete string, directive cobra.ShellCompDirective) ([]cobra.Completion, cobra.ShellCompDirective) {
	completionSource := runtimeServicesFrom(cmd).ManagedAppCompletions
	if completionSource == nil {
		return nil, directive
	}
	rows, err := completionSource(toComplete)
	if err != nil {
		return nil, directive
	}
...
func completeManagedAppIDTarget(cmd *cobra.Command, args []string, toComplete string) ([]cobra.Completion, cobra.ShellCompDirective) {
	...
	return completeManagedAppIDs(cmd, toComplete, cobra.ShellCompDirectiveNoFileComp)
}
